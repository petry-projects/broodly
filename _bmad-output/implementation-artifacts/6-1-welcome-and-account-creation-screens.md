...
As a new user,
I want to see a clear value proposition on the welcome screen and create my account with Google or Apple Sign-In,
...
3. GIVEN the account creation screen WHEN I tap "Continue with Google" or "Continue with Apple" THEN the OAuth flow completes via Firebase Auth and I proceed to step 2.
4. GIVEN the account creation screen WHEN I do not check the Terms of Service and Privacy Policy checkbox THEN the social sign-in buttons are disabled.
5. GIVEN no network connectivity WHEN I am on the welcome screen and tap "Get Started" THEN local-only onboarding begins with a subtle info banner explaining account creation will complete when online.
6. GIVEN the welcome screen WHEN I tap "Sign In" THEN I navigate to the existing sign-in screen (Epic 5 auth flow).
...
- [ ] Create account creation screen at `apps/mobile/app/(onboarding)/create-account.tsx` (AC: #3, #4)
  - [ ] Render progress dots component showing step 1 of 5 (dot-active for step 1)
  - [ ] Render "Continue with Google" button (branded, 48px height)
  - [ ] Render "Continue with Apple" button (branded, 48px height, required on iOS)
  - [ ] Render ToS/Privacy checkbox using Gluestack Checkbox with linked text
  - [ ] Social sign-in buttons disabled when ToS unchecked
...
- [ ] Implement offline-safe local account deferral (AC: #5)
  - [ ] Detect connectivity via NetInfo
  - [ ] When offline, store profile data in Zustand onboarding store and show info Alert
...
- [ ] Write unit tests (AC: all)
  ...
  - [ ] Test: Social sign-in buttons are disabled when ToS checkbox is unchecked
...
- Social login only (Google + Apple Sign-In) — no email/password forms
...
- **Firebase methods:** `signInWithCredential` (Google/Apple providers)
...
- Do NOT build custom input components; use Gluestack components
- Do NOT implement email/password fields — social login only
